...
def main():
    ...
    ev_parser.add_argument("buttons_remaining", type=int, help="Number of buttons remaining")
    ev_parser.add_argument("players_without_button", type=int, help="Number of other players without a button")
    
    # Command 2: genie
    genie_parser = subparsers.add_parser("genie", help="Calculate max amount to pay genie")
    genie_parser.add_argument("buttons_owned", type=int, help="Number of buttons you have")
    genie_parser.add_argument("buttons_remaining", type=int, help="Number of buttons remaining")
    genie_parser.add_argument("players_without_button", type=int, help="Number of other players without a button")
    
    # Command 3: matrix
    matrix_parser = subparsers.add_parser("matrix", help="Print 2D matrix for given buttons remaining")
    matrix_parser.add_argument("buttons_remaining", type=int, help="Number of buttons remaining")
    
    args = parser.parse_args()
    
    if args.command == "ev":
        ev = expected_value(args.buttons_owned, args.buttons_remaining, args.players_without_button)
        print(f"EV(buttons_owned={args.buttons_owned}, buttons_remaining={args.buttons_remaining}, players_without_button={args.players_without_button}) = {ev:.2f}")
    elif args.command == "genie":
        amount = calculate_genie_amount(args.buttons_owned, args.buttons_remaining, args.players_without_button)
        print(f"Max genie payment(buttons_owned={args.buttons_owned}, buttons_remaining={args.buttons_remaining}, players_without_button={args.players_without_button}) = {amount:.2f}")
